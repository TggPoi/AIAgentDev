...
//本地测试用的工具函数，实际项目中应该放在 service 里，并注入到 AiService 中
...
  private readonly modelWithTools: Runnable<BaseMessage[], AIMessage>;
  ...
  constructor(
    ...
    @Inject('QUERY_USER_TOOL') private readonly queryUserTool : any,//把查询用户的工具更改为provider注入的方式，实现provider的Tool调用Service中的方法
  ...
      this.modelWithTools = model.bindTools([this.queryUserTool,this.sendMailTool, this.webSearchTool, this.dbUsersCrudTool, this.cronJobTool, this.timeNowTool]);
    }
...
注意：像“\`1分钟后提醒我喝水\`”，时间相关信息用于计算下一次执行时间，而 \`instruction\` 应该是“提醒我喝水”；本轮不需要立刻提醒。`,
  ...
              }),
            );
          } else if (toolName === 'send_mail') { //调用邮箱工具
          ...
          } else if (toolName === 'web_search') {//网络搜索工具
          ...
          } else if (toolName === 'db_users_crud') {//数据库增删改查工具
          ...
          } else if (toolName === 'cron_job') {//定时任务管理工具
          ...
          } else if (toolName === 'time_now') {//获取当前时间工具
  ...
  async *runChainStream(query: string): AsyncIterable<string> {
    ...
    注意：像“\`1分钟后提醒我喝水\`”，时间相关信息用于计算下一次执行时间，而 \`instruction\` 应该是“提醒我喝水”；本轮不需要立刻提醒。`,
      ...
         }
        ...
        console.error('[runChainStream error]', error);
        throw error;
      }
   }
}