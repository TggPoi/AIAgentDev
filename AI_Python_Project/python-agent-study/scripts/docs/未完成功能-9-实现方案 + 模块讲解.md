...
# 【讲解】RuntimeRecord 模块的作用

~~~cpp
//简单概括RuntimeRecord 是一张“任务恢复索引/运行登记表”，TaskPlan 能够找到 task_plan_id，再用它查找 RuntimeRecord；随后通过 RuntimeRecord 的 thread_id 找到 Checkpoint，用户当时的权限状态，文档状态

//触发retry接口时，就会先找到 task_plan_id 里面的 RuntimeRecord，再进行校验恢复
~~~
...
> 每个 Deep Document Agent 任务的一张“运行登记卡”。

它记录这个任务当前运行到什么状态、对应哪个 LangGraph Checkpoint，以及恢复前需要检查哪些安全事实。
...
此时系统会为这个任务保存一条 RuntimeRecord，大致如下：

```py
{
  "task_plan_id": "task_plan_001",
  "thread_id": "document:task_plan_001",
  "status": "failed",
  "record_version": 5,
  "resume_count": 0,
  "acl_fingerprint": "用户当前权限指纹",
  "candidates": {
    "doc_123": {
      "doc_id": "doc_123",
      "source_path": "development/gitlab-guide.md"
    }
  },
  "read_snapshots": {
    "doc_123": {
...
记录对应的 LangGraph Checkpoint 线程：

```
document:task_plan_001
```

RuntimeRecord 自己不保存 LangGraph 的消息和草稿，而是通过 `thread_id` 告诉系统：

> 这个任务的真正 Checkpoint 应该去哪里找。

### `status`

```
status: running | failed | cleanup_pending
```

表示运行记录当前状态：

- `running`：Agent 正在运行；
- `failed`：本次执行失败，但现场可能仍然存在；
- `cleanup_pending`：任务结束了，但 Checkpoint 清理失败，等待后续清理。
...
每次修改这张“运行登记卡”，版本号都会增加：

```
1 → 2 → 3 → 4
```

它只保护 RuntimeRecord，不是 LangGraph Checkpoint 的版本。
...
```
原来的 ACL 指纹 != 当前 ACL 指纹
```

说明用户权限发生变化，旧 Checkpoint 中可能包含用户现在已经不能访问的文档，因此不能继续使用旧现场。
...
记录 Researcher 通过当前 ACL 检索实际获得的文档候选。
...
这样恢复后，Writer 或后续服务端校验仍然能够证明：

```
doc_123 是本轮经过权限过滤检索得到的
```
...
记录 Researcher 真正读取过哪些文档，以及读取时的 SHA。
...
说明文档在任务失败后被其他人修改了。旧 Writer 推理基于旧文档，因此旧 Checkpoint 不能直接继续使用。
...
它不是相信模型说“我调用过工具”，而是由工具执行代码在成功后登记。
...
记录这个任务已经从 Checkpoint 恢复过多少次。

例如：

```
第一次执行失败：resume_count = 0
第一次 retry：resume_count = 1
第二次 retry：resume_count = 2
```
...
这样失败任务不会永久占用 PostgreSQL 存储。

## RuntimeRecord 和 Checkpoint 的区别

这是最关键的部分。

| 内容                  | RuntimeRecord | LangGraph Checkpoint           |
| --------------------- | ------------- | ------------------------------ |
| 任务 ID               | 有            | 通过 `thread_id` 关联          |
| 运行状态              | 有            | 不是主要职责                   |
| ACL 指纹              | 有            | 不作为恢复安全判断入口         |
| 文档路径和 SHA        | 有            | 可能存在于图状态，但不直接信任 |
| 恢复次数              | 有            | 没有业务层含义                 |
| Agent 消息            | 没有          | 有                             |
| 当前节点              | 没有          | 有                             |
| Todo                  | 没有          | 有                             |
| Researcher 摘要       | 没有完整内容  | 有                             |
| Writer 草稿           | 没有          | 有                             |
| StateBackend 虚拟文件 | 没有          | 有                             |

可以简化为：

```
RuntimeRecord
回答：这个 Checkpoint 现在是否允许恢复？

LangGraph Checkpoint
回答：Agent 应该从哪个节点和状态继续执行？
```

## 【重点】为什么只有 Checkpoint 还不够

假设系统只有 LangGraph Checkpoint：

```
Checkpoint 中保存了 Researcher 消息和 Writer 草稿
```

用户点击重试时，系统虽然能找到旧状态，但无法方便、可信地判断：

- 当前用户权限是否已经变化；
- **Researcher 读取的文档是否已更新**；
- 这条任务是否正在被另一个请求恢复；
- 这是第几次恢复；
- 失败现场是否已经过期；
- 哪些候选 `doc_id` 是服务端检索登记的；
- 哪些工具确实执行成功。

因此增加 RuntimeRecord，作为恢复前的安全登记表。
...
flowchart TD
    A["用户点击 retry"] --> B["读取 RuntimeRecord"]
    B --> C{"当前权限与 ACL 指纹一致？"}
    C -- "否" --> X["废弃旧 Checkpoint"]
    C -- "是" --> D{"源文档 SHA 一致？"}
    D -- "否" --> X
    D -- "是" --> E{"Checkpoint 存在？"}
    E -- "否" --> F["返回 Checkpoint 不可用错误"]
    E -- "是" --> G["RuntimeRecord 标记 running"]
    G --> H["使用 thread_id 读取 LangGraph Checkpoint"]
    H --> I["从最近完整节点继续"]
...
[deep_document_runtime.py (line 329)](D:/AI_Agent_Project/AI_Python_Project/python-agent-study/src/fast_app/services/agent_tasks/deep_document_runtime.py:329)
...
```
runtime = DeepDocumentRuntime(
    checkpointer=saver,
    store=store,
)
```

两者共用 PostgreSQL 和连接池，但用途不同：
...
不要把 RuntimeRecord 理解成“另一份 Checkpoint”。

它更像一张**索引卡：**

```
任务：task_plan_001
对应 Checkpoint：document:task_plan_001
当前状态：failed
是否允许恢复：需要重新检查 ACL 和文档 SHA
已经恢复：0 次
服务端确认读取过：doc_123
记录版本：5
保留到：2026-08-04
...
都在 LangGraph Checkpoint 中。

一句话总结：

> RuntimeRecord 是业务层的“恢复登记与安全校验记录”；Checkpoint 是 LangGraph 的“实际执行现场”。系统先读取 RuntimeRecord 判断能不能恢复，再根据其中的 `thread_id` 找到 Checkpoint，从最近完整节点继续执行。
