...
目前只有 [test_rag_chat_api.py (line 896)](D:/AI_Agent_Project/AI_Python_Project/python-agent-study/scripts/tests/rag_memory/test_rag_chat_api.py:896) 的人工 LangSmith 场景脚本，没有针对以下逻辑的最小自动测试：
...
## ⚠️ 问题十二：多 Worker / 多实例下 Agent TaskPlan 的重复执行与状态一致性问题：

[Agent TaskPlan 多 Worker 一致性问题分析](D:\AI_Agent_Project\AI_Python_Project\python-agent-study\scripts\docs\【重要问题】多Worker实例时的多线程问题.md)

[Agent TaskPlan 多 Worker / 多实例一致性完整解决方案（含完整代码、迁移、并发回归与 10–15 人压测）](<D:\AI_Agent_Project\AI_Python_Project\python-agent-study\scripts\docs\问题十二-Agent TaskPlan多Worker一致性完整解决方案.md>)

当前问题不等同于“多人使用”或普通 Python 多线程不安全。单 FastAPI 进程内，同一 `task_plan_id` 的首次执行、retry/resume 和 confirm 已有进程内锁并返回稳定的 `HTTP 409 / AGENT_TASK_PLAN_BUSY`；真正未解决的是同一 TaskPlan 落到不同 Uvicorn Worker 或不同应用实例时，进程内锁无法互斥，并且 TaskPlan 快照、取消终态与 `record_version` 更新缺少数据库原子 CAS/条件状态转换。

在完成 PostgreSQL TaskPlan 事实表、数据库租约、原子 CAS 和单调状态转换前，应保持单 Uvicorn Worker、单 FastAPI 实例。系统可支持多少并发用户或复杂 Agent 尚未经过真实压测，不能由“注册用户数”直接推断。

[Deepseek修订后的方案，GPT的方案有问题：](file:///D:/AI_Agent_Project/AI_Python_Project/python-agent-study/scripts/docs/%E9%97%AE%E9%A2%98%E5%8D%81%E4%BA%8C-Agent%20TaskPlan%E5%A4%9AWorker%E4%B8%80%E8%87%B4%E6%80%A7%E6%96%B9%E6%A1%88-%E5%AD%A6%E4%B9%A0%E9%A1%B5.html)
...
## ✅ 【重要】问题十五：TaskPlan拆解质量需要提升：

[升级方案 和 经验](D:\AI_Agent_Project\AI_Python_Project\python-agent-study\scripts\docs\TaskPlan 生成质量修复与 Planner 质量门禁重构方案.md)

[测试报告文档，bug解决方案讲解](D:\AI_Agent_Project\AI_Python_Project\python-agent-study\scripts\docs\TaskPlan真实模型Web测试过程与问题记录.md)

修复时遇到的子问题：[web检索质量提升技术点](D:\AI_Agent_Project\AI_Python_Project\python-agent-study\scripts\docs\TaskPlan--web搜索质量提升技术方案：.md)



# 未完成的功能：

## ✅ 0. 【重点】未实现tool并行执行 【使用DAG】
...
现有功能完成后，评估目前工程的检索质量，使用 **Eval模块** 量化，通过生成的报告评估

但是完整构建eval模块工作量太大，token完全不够用，目前方案中的阶段只做了一部分，暂时封档：[Eval模块构建方案](D:\AI_Agent_Project\AI_Python_Project\python-agent-study\learning-docs\phase-11\Eval完整评估模块待办.md)

目前使用替代方案，引入外部框架，而不是从头开始构建，先用着：[DeepEval实现方案](D:\AI_Agent_Project\AI_Python_Project\python-agent-study\scripts\docs\Rag-Eval 模块技术方案：.md)
...
[测试验收文档](D:\AI_Agent_Project\AI_Python_Project\python-agent-study\scripts\docs\NL2SQL测试过程与问题记录.md)

[代码模块拆解讲解](D:\AI_Agent_Project\AI_Python_Project\python-agent-study\scripts\docs\NL2SQL自然语言转SQL实现教程.md)
...
目前接入的 qwen plus模型是能识别图片的



# 严重报错：

## ✅ 1、【核心错误】多Agent模块无法完成复杂Agent任务：【已修复】
...
[经验总结 + 本次修复使用的 技术知识点讲解](D:\AI_Agent_Project\AI_Python_Project\python-agent-study\scripts\docs\多Agent端到端测试复盘 + 知识点讲解.md)
