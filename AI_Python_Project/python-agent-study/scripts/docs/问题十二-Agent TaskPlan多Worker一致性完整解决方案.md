# 问题十二：Agent TaskPlan 多 Worker / 多实例一致性完整解决方案

> 文档性质：实施设计与代码清单。本文中的新增代码尚未写入业务模块、尚未执行测试；实施 Agent 必须严格按本文顺序完成 RED → GREEN → 压测，不能把代码示例当成已经验收的现状。
...
4. `cancelled` 是受保护终态，旧执行者不能把它改回 `running/completed/failed`。
...
```text
created → running | waiting_confirmation | failed | cancelled
running → running | waiting_confirmation | completed | completed_with_warnings | failed | cancelled
...
`running → running` 用于进度快照；它仍然必须增加 `record_version`。
...
ALLOWED_SOURCE_STATUSES_BY_TARGET: dict[AgentTaskPlanStatus, set[AgentTaskPlanStatus]] = {
    ...
    AgentTaskPlanStatus.COMPLETED: {AgentTaskPlanStatus.RUNNING},
    AgentTaskPlanStatus.COMPLETED_WITH_WARNINGS: {AgentTaskPlanStatus.RUNNING},
    AgentTaskPlanStatus.FAILED: {
        AgentTaskPlanStatus.CREATED,
        AgentTaskPlanStatus.RUNNING,
    },
...
这里故意不允许任何目标状态从 `completed` 或 `cancelled` 转出。`completed_with_warnings` 仍可进入 `running`，与当前 Research retry 语义保持兼容。
        ...
        *,
        ...
        async with self._session_factory() as session, session.begin():
            ...
            if inserted is None:
                ...
                raise AgentTaskPlanIdempotencyConflictError(
                    "该幂等请求已失败或被拒绝；修正原因后必须使用新的 Idempotency-Key"
                )
...
实现时还要在同一个 Repository 中增加两个小方法，不允许回到 `PostgresStore`：
    ...
    async def resume(
        ...
        async with _TASK_PLAN_LOCKS.hold(task_plan_id):
            ...
            allowed = (
                ...
                if is_research
                else {AgentTaskPlanStatus.RUNNING, AgentTaskPlanStatus.FAILED}
            )
...
`alembic current` 和 `heads` 都必须指向唯一的 `20260813_0013`。若项目在实施期间已经新增迁移，不能继续硬编码 0013；应基于当时真实 head 重新生成 revision，并同步本文文件名和命令。
...
这一测试只接受 `1 个 2xx + 19 个 AGENT_TASK_PLAN_BUSY`。它还要回读最终 TaskPlan，防止“只有一个请求返回成功，但任务事实仍卡在 running”。日志中若出现两个真实 ToolCall、两个 MR 或两个发布版本，即使 HTTP 断言通过也必须判失败并检查副作用幂等键。
...
async def run(limit: int) -> tuple[int, int, int]:
    settings = get_settings()
    engine = create_database_engine(settings)
    repository = AgentTaskPlanRepository(create_session_factory(engine))
    ...
    try:
        ...
        for task_plan_id in task_ids:
            ...
            if record is None:
                continue
            idempotency_key = (
                f"cleanup:{task_plan_id}:{int(record['record_version'])}"
            )
            try:
...
def main() -> None:
    ...
    args = parser.parse_args()
    cleaned, skipped, commands_deleted = asyncio.run(run(args.limit))
    print(
        f"cleaned={cleaned} skipped={skipped} "
        f"commands_deleted={commands_deleted}"
    )
