...
const exampleSelector = await LengthBasedExampleSelector.fromExamples(examples, {
  examplePrompt,
  // 这里简单地用字符长度近似控制，真实项目中可以配合 token 估算 
  maxLength: 700,//长度的计算规则=> getTextLength: (text) => text.length，文本长度超过 700 就不选了
...
/**
 * PS D:\AI_Agent_Project\prompt-template-test> node .\src\example-selector-length.mjs
下面是一些不同风格和长度的周报片段示例，你可以从中学习语气和结构：
