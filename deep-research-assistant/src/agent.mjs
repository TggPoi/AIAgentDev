// path：Node.js 路径工具，用于拼接项目目录和记忆文件路径。
import path from "node:path";
// fileURLToPath：将当前 ES Module 的 URL 转换为本地文件路径。
import { fileURLToPath } from "node:url";
// dedent：移除多行提示词的公共缩进，使提示词便于在代码中排版。
import dedent from "dedent";
// ChatOpenAI：创建兼容 OpenAI API 的聊天模型实例。
import { ChatOpenAI } from "@langchain/openai";
// createCodeInterpreterMiddleware：为分析师子 Agent 提供 JavaScript REPL。
import { createCodeInterpreterMiddleware } from "@langchain/quickjs";
// createDeepAgent：创建主 Agent；FilesystemBackend：提供虚拟工作区文件系统。
import { createDeepAgent, FilesystemBackend } from "deepagents";

// webSearch：研究员子 Agent 使用的联网搜索工具。
import { webSearch } from "./tools/search.mjs";

// projectDir：工程根目录，供虚拟文件系统、记忆文件和 CLI 输出路径共同使用。
const projectDir = path.resolve(
  path.dirname(fileURLToPath(import.meta.url)),
  "..",
);

//dedent 用于去掉换行和缩进的空格，换成 \n
// researcherSubAgent：研究员子 Agent 配置，由 createIntelligenceDeskAgent 传给 createDeepAgent。
...
// editorSubAgent：编辑子 Agent 配置，由 createIntelligenceDeskAgent 传给 createDeepAgent。
...
// analystSubAgent：分析师子 Agent 配置，由 createIntelligenceDeskAgent 传给 createDeepAgent。
const analystSubAgent = {
  ...
  `,
  middleware: [createCodeInterpreterMiddleware()], //执行js代码的js引擎，让Agent可以执行代码
};

// orchestratorPrompt：主 Agent 的系统提示词，规定调研、分析、起草、审阅和定稿流程。
const orchestratorPrompt = dedent`
  ...
  **仅**以下 subagent_type 合法：researcher、analyst、editor、general-purpose。
  ...
  - 每个调研员只负责一个聚焦的子主题
  - **每份报告最多 2 个调研员**——只选最相关的子主题
  - 框架对比类任务：优先调研用户明确点名的框架；否则选最重要的 2 个
  - 最多并行启动 2 个调研员，已有 2 份 findings 文件后不再新增调研员
  - 仅在确实需要数值计算时使用分析师
  - 每份报告只调用编辑一次（草稿完成后）
...
// createIntelligenceDeskAgent：创建可执行的深度调研 Agent。
// 调用位置：src/cli.mjs 的 run()。
export function createIntelligenceDeskAgent() {
  // apiKey：OpenAI API 密钥；缺失时无法创建可调用的模型。
  const apiKey = process.env.OPENAI_API_KEY?.trim();
  if (!apiKey) {
    throw new Error("未设置 OPENAI_API_KEY 环境变量");
  }

  // model：模型名称，允许通过环境变量覆盖默认值。
  const model = process.env.OPENAI_MODEL?.trim() || "gpt-4o";
  // baseURL：可选的 OpenAI 兼容接口地址。
  const baseURL = process.env.OPENAI_BASE_URL?.trim() || undefined;

  // backend：将工程根目录映射为 Agent 可访问的虚拟工作区。
  ...
  // chatModel：主 Agent 和子 Agent 共用的聊天模型实例。
  ...
  /*
  maxInputTokens设置的太小可能导致：
- 很快触发摘要；
- 摘要频繁；
- 细节丢失；
- 流程约束被压缩掉。导致Editor阶段被略过
*/
  // profile getter：覆盖模型输入上限，供 LangChain 读取 chatModel.profile 时调用。
  //设置上下文压缩触发阈值为8000，这样在输入超过8000 tokens时会自动进行上下文压缩，保留重要信息以适应模型的输入限制
