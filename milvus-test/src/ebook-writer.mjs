  ...
  try {
    ...
    if (!hasCollection.value) {
      ...
      await client.createCollection({
        collection_name: COLLECTION_NAME,
        fields: [ //chapter_num（第几章）、index（第几个分块）、content（内容）、vector（向量） book_id 这个是用来和 mysql 里 book 表关联的，这里暂时不用
...
async function loadAndProcessEPubStreaming(bookId) {
  try {
    ...
    // 使用 EPubLoader 加载文件，按章节拆分 splitChapters: true
    ...
    // 上面按照章节拆分还是太长了，创建文本拆分器，拆分到 500 个字符
    ...
    for (let chapterIndex = 0; chapterIndex < documents.length; chapterIndex++) {
      ...
      // 使用 splitter 进行二次拆分，把已经拆分好的单个章节的内容再拆分成更小的chunk，拆分到 500 个字符
