...
/**
 * 这是一个增强版的 RAG 示例，增加了一个路由节点来判断问题是否需要检索，以及一个评估节点来判断本地检索结果是否足够，不足则进行联网搜索补充。
 */
 ...
 * 搜索 API 的调用逻辑封装在这里，返回格式化的搜索结果文本供后续评估和生成节点使用。
 */
...
//联网搜索节点，基于用户问题进行网络搜索，补充检索到的本地知识库可能没有的信息，搜索结果会被存储在 state.webContext 里供后续评估和生成节点使用
const webSearchNode = async (state) => {
  ...
  //如果评估节点里模型已经给出了适合的联网搜索查询，就用这个查询；否则就用用户原始问题进行搜索
