...
/**
 * 这种方式比 output parser 更好,因为模型训练的时候就保证了生成 tool calls 的参数一定是符合格式要求的，如果不符合，会重新生成。而 output parser 是在模型生成文本后再去解析，如果模型生成的文本格式不对，解析就会失败。
 * output parser会占用一堆提示词的Token，而且还不能保证结果的格式一定正确
 * 
...
/**
 * PS D:\AI_Agent_Project\output-parser-test> node .\src\tool-call-args.mjs                  
response.tool_calls: [
  {
    name: 'extract_scientist_info',
    args: {
    ...
    type: 'tool_call',
    id: 'call_be1b0b02977d4047ae3fa0a1'
  }
]
结构化结果: {
  "name": "阿尔伯特·爱因斯坦",
