...
const addContextMiddleware = createMiddleware({
  ...
  //wrapModelCall的调用时机是在模型调用前，可以修改传入模型的请求参数
...
/**
 * PS D:\AI_Agent_Project\deepagents-test> node .\src\middleware-test.mjs
...
[Logging] 模型返回: Middleware（中间件）是一种在客户端和服务器之间处理请求与响应的软件层，用于执行如日志记录、身份验证、数据解析等通用任务。...
[Logging] agent 结束，累计模型调用: 1 次

回复: Middleware（中间件）是一种在客户端和服务器之间处理请求与响应的软件层，用于执行如日志记录、身份验证、数据解析等通用任务。
modelCallCount: 1
