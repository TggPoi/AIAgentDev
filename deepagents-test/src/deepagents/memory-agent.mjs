...
/**
 * PS D:\AI_Agent_Project\deepagents-test> node .\src\deepagents\memory-agent.mjs  

用户: 根据记忆，这个项目是做什么的？只答一句。
回复: 项目是做项目助手，负责文件系统操作（如 `ls`、`read_file`、`edit_file` 等）并根据用户指令管理项目说明、技术栈、架构及用户偏好等记忆。

用户: 请记住：我常用的包管理器是 pnpm。
回复: 已记住：你常用的包管理器是 pnpm。

用户: 请记住：本仓库主入口脚本是 src/deepagents/memory-agent.mjs。
回复: 已记住：本仓库主入口脚本是 `src/deepagents/memory-agent.mjs`。

用户: 我常用什么包管理器？本 demo 主入口脚本路径是什么？各用一行回答。
回复: pnpm  
src/deepagents/memory-agent.mjs

--- /AGENTS.md ---
 ## Entry Point
- `src/deepagents/memory-agent.mjs` is the main entry script.
